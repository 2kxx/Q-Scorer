...
> **Tip**: The LoRA-only release at [`2kxx/Qscorer_lora_5_1`](https://huggingface.co/2kxx/Qscorer_lora_5_1) is the `LoRA adapter + non_lora_trainables.bin` format and **cannot** be loaded directly via `AutoModelForCausalLM`. Use it through [`scorer.py`](#local-scorerpy) (which needs the base mPLUG-Owl2 weights too), or [merge it locally](#producing-a-merged-checkpoint) and load the merged folder via the one-liner above.
...
```bash
pip install -U "huggingface_hub[cli]"   # ships the new `hf` CLI (`huggingface-cli` is deprecated)
...
```bash
hf repos create <your-username>/Qscorer_merged --repo-type model
```

**3. Upload the merged folder**

`hf upload` handles git-lfs for the weight shards automatically:

```bash
hf upload <your-username>/Qscorer_merged \
...
- `tokenizer.model`, `special_tokens_map.json`, `added_tokens.json`, `tokenizer_config.json`
...
- `README.md` — auto-generated by the merge script so the Hub renders a model card out of the box. (A `MODEL_CARD.md` copy is also written for backward compatibility.)
